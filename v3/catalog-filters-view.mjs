...
const titles = {category:'Catégorie',brand:'Marque',color:'Couleur',material:'Matière',usage:'Usage',feature:'Caractéristiques'};
...
export function chairParams(state, page = state.page) {
  ...
  for(const key of ['category','brand','color','material','usage','feature']) if(state[key]?.length) q.set(key,state[key].join(','));
...
function listFilter(key, data) {
  ...
  return `<details class="catalog-filters__group" data-filter-group="${key}"><summary>${titles[key]}${selected.length?`<span class="catalog-filters__selected">${selected.length}</span>`:''}<span class="catalog-filters__chevron" aria-hidden="true"></span></summary>
    <div class="catalog-filters__popover">${values.length>8?`<label class="catalog-filters__search">Rechercher une ${key==='brand'?'marque':key==='category'?'catégorie':'valeur'}<input type="search" data-filter-search="${key}" placeholder="Rechercher…" autocomplete="off"></label>`:''}<div class="catalog-filters__options">${list}</div><button type="submit" class="btn btn--outline" data-filter-apply>Appliquer</button></div></details>`;
}
  ...
  return `<div class="catalog-filters__heading"><h2 class="serif catalogue-head">${esc(scope.heading)}</h2><span class="plp-count" data-chair-count>${total} modèle${total>1?'s':''}</span></div>
    <form class="catalog-filters" action="${esc(scope.basePath)}#grille" method="get" aria-label="${esc(scope.formLabel)}">
      ...
      <div class="catalog-filters__groups" id="chair-filter-options">
        ...
        <label class="catalog-filters__sort"><span class="sr-only">${esc(scope.sortLabel)}</span><select class="fselect" name="sort">${[['pop',scope.popLabel||'Les plus populaires'],['asc','Prix croissant'],['desc','Prix décroissant'],['az','Nom : A → Z']].map(([v,l])=>`<option value="${v}"${state.sort===v?' selected':''}>${l}</option>`).join('')}</select></label>
        ${listFilter('category',data)}${listFilter('brand',data)}
