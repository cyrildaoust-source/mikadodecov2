...
// Le répertoire suit les mêmes sources et règles que la sélection marque.
// Une nouvelle marque publiée apparaît sans ajouter de carte éditoriale.
async function collectionBrands(handle, fetchPage) {
  const counts = new Map(), seen = new Set();
  let collection;
  for (const source of collectionSources(handle)) {
    let cursor = null;
    const visited = new Set();
    do {
      if (visited.has(cursor)) throw new Error('Collection cursor did not advance');
      visited.add(cursor);
      const page = await fetchPage(100, cursor, source);
      if (!page) break;
      collection ||= { ...page.collection, handle, title: families[handle]?.title || ({ sieges: 'Assises', outdoor: 'Jardin' })[handle] || page.collection.title };
      for (const product of page.items) {
        const id = product.id || product.handle;
        if (seen.has(id) || !inFamily(handle, product) || !product.brand) continue;
        seen.add(id);
        const slug = slugify(product.brand);
        const entry = counts.get(slug) || { slug, name: brandName(slug, [{ name: product.brand }]), productCount: 0 };
        entry.productCount++;
        counts.set(slug, entry);
      }
      if (page.pageInfo?.hasNextPage && !page.pageInfo.endCursor) throw new Error('Missing collection cursor');
      cursor = page.pageInfo?.hasNextPage ? page.pageInfo.endCursor : null;
    } while (cursor);
  }
  return collection ? { collection, brands: [...counts.values()].sort((a, b) => a.name.localeCompare(b.name, 'fr')) } : null;
}
...
module.exports = { brandCollectionPage, brandName, collectionBrands };
