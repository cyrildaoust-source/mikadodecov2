...
const FILTER_FIELDS = [
  ...
  'custom.width_cm','custom.depth_cm','custom.height_cm','custom.seat_height_cm',
...
const variantExtra = `pageInfo { hasNextPage endCursor }
  edges { node { quantityAvailable
    filterMetafields: metafields(identifiers: [${identifiers(['custom.color_family','custom.material_family'])}]) {
      namespace key type value
    }
  } }`;

function chairQuery(cardFragment) {
  return `${cardFragment.replace('variants(first: 250)', 'variants(first: 100)')}
    query ChairCatalog($after: String) {
      collection(handle: "chaises") {
...
async function readChairCatalog(fetchPage, mapProduct, fetchVariants) {
  ...
  do {
    if(cursors.has(after)) throw new Error('Chair collection cursor did not advance');
    cursors.add(after);
    const data = await fetchPage(after);
    if(!data?.products?.edges || !data.products.pageInfo) throw new Error('Chair collection unavailable');
    ...
    for(const {node} of data.products.edges) {
      ...
      while(node.variants.pageInfo?.hasNextPage) {
        ...
        if(!cursor || vc.has(cursor) || !fetchVariants) throw new Error('Incomplete chair variants');
        vc.add(cursor);
        const more = await fetchVariants(node.handle,cursor);
        if(!more?.edges || !more.pageInfo) throw new Error('Chair variants unavailable');
    ...
    if(!after) throw new Error('Missing chair collection cursor');
  } while(true);
  return {collection,products};
}

module.exports = {chairQuery,VARIANT_QUERY,readChairCatalog,filterCatalog,FILTER_FIELDS};
