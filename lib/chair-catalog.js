...
// Tout le catalogue publié, dans l'ordre des ventes : source de l'index commun.
function catalogueQuery(cardFragment) {
  return `${cardFragment.replace('variants(first: 250)', 'variants(first: 100)')}
    query CatalogueIndex($after: String) {
...
module.exports = {scopeQuery,catalogueQuery,chairQuery,VARIANT_QUERY,readChairCatalog,readScopeCatalog:readChairCatalog,filterCatalog,FILTER_FIELDS};
