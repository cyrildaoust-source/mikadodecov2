...
function renderFamilyPage(template, handle, { items = [], pageInfo = {}, failed = false, cards = '', cursor = '', iconItems = [], iconCards = '' } = {}) {
  ...
  const replacements = {
    ...
    ICONS: family.icons === false ? '' : `<section class="sec" data-icones-sec${iconCards ? '' : ' hidden'} aria-labelledby="family-icons"><h2 class="lab" id="family-icons">${escapeHtml(family.icons?.title || 'Les icônes')}</h2><div class="fam-icon-rail${iconItems.length && iconItems.length < 3 ? ' fam-icon-rail--compact' : ''}" data-icones data-famrail role="region" aria-labelledby="family-icons">${iconCards}</div></section>`,
    ...
    INITIAL: jsonForHtml({ handle, items: items.map(cardSeed), pageInfo, cursor, pageSize: PAGE_SIZE, failed, curatedIcons: !!family.icons?.handles, iconItems: iconItems.map(cardSeed) }),
  };
