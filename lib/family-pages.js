...
function rail(categories, { id = 'family-categories', title = 'Nos catégories', compact = false } = {}) {
  const fit = categories.length <= 5 ? ' home-rail--fit' : '';
  return `<section class="${compact ? 'sec' : 'section wrap'}" aria-labelledby="${escapeHtml(id)}">
    <div class="home-railhead"><h2 class="serif" id="${escapeHtml(id)}">${escapeHtml(title)}</h2>
    </div>
    <div class="home-rail${fit}" data-famrail role="region" aria-labelledby="${escapeHtml(id)}">${categories.map(category => `<a class="home-rc" href="${familyUrl(category.handle)}"><img class="home-rc__img editorial-photo" style="${photoStyle(category)}" src="${escapeHtml(category.image)}" alt="" loading="lazy"><span class="home-rc__scrim" aria-hidden="true"></span><span class="home-rc__n">${escapeHtml(category.title)}</span></a>`).join('')}</div>
  </section>`;
}
...
function brands(family, handle) {
  return `<section class="sec" aria-labelledby="family-brands"><h2 class="lab" id="family-brands">Nos marques</h2><div class="bgrid">${family.brands.map(brand => `<a class="bcard" href="${familyUrl(handle)}?brand=${encodeURIComponent(brand.slug)}" aria-label="${escapeHtml(family.title + ' · ' + brand.name)}"><span class="ph"><img class="ph-img editorial-photo" style="${photoStyle(brand)}" src="${escapeHtml(brand.image)}" alt="" loading="lazy"></span><span class="bcard__scrim"></span><span class="bcard__logo"><img src="${escapeHtml(brand.logo || `/images/brands/${brand.slug}.svg`)}" alt="${escapeHtml(brand.name)}" loading="lazy"></span></a>`).join('')}</div></section>`;
}

// computed : sélection déjà calculée par le serveur (vide ou non) ; le navigateur ne la relit pas.
function iconSection(title, cards = '', computed = false) {
  return `<section class="sec" data-icones-sec${computed ? ' data-ssr' : ''}${cards ? '' : ' hidden'} aria-labelledby="family-icons"><h2 class="lab" id="family-icons">${escapeHtml(title)}</h2><div class="fam-icon-rail" data-icones data-famrail role="region" aria-labelledby="family-icons">${cards}</div></section>`;
}
...
function renderFamilyPage(template, handle, { items = [], pageInfo = {}, failed = false, cards = '', cursor = '', featuredItems = [], featuredCards = '', iconCards = null } = {}) {
  ...
  const replacements = {
    ...
    URL: escapeHtml('https://www.mikadodeco.be' + familyUrl(handle) + (cursor ? '?cursor=' + encodeURIComponent(cursor) : '')),
    ...
    ICONS: family.icons === false ? '' : iconSection('Les icônes', iconCards || '', iconCards !== null),
...
module.exports = { families, seatingIcons, PAGE_SIZE, nextPageUrl, renderFamilyPage, renderSeatingPage, renderCategoryRail: rail, cardSeed };
