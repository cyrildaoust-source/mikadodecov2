...
function brands(family) {
  return `<section class="sec" aria-labelledby="family-brands"><h2 class="lab" id="family-brands">Nos marques</h2><div class="bgrid">${family.brands.map(brand => `<a class="bcard" href="/produits.html?brand=${encodeURIComponent(brand.slug)}"><span class="ph"><img class="ph-img editorial-photo" style="${photoStyle(brand)}" src="${escapeHtml(brand.image)}" alt="" loading="lazy"></span><span class="bcard__scrim"></span><span class="bcard__logo"><img src="/images/brands/${escapeHtml(brand.slug)}.svg" alt="${escapeHtml(brand.name)}" loading="lazy"></span></a>`).join('')}</div></section>`;
}
