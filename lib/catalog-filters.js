...
// Mots de recherche d'une finition (recalculés au chargement de l'index commun).
function variantSearchWords(card, v) {
  const labelOptions = (v.options || []).filter(o => o.value && o.value !== 'Default Title');
  return normalize([card.name, card.brand, v.title, ...labelOptions.map(o => o.value)].join(' ')).split(' ');
}
function buildFilterProduct(raw, card) {
  ...
  const variants = card.variants.map((v, i) => {
    ...
    const searchWords = variantSearchWords(card, v);
...
const LIST_KEYS = ['category','brand','color','material','usage','feature'];
...
// La même recherche est testée sur chaque fiche : la normaliser une seule fois.
let lastQuery = null, lastWords = [];
const queryWords = q => q === lastQuery ? lastWords : (lastQuery = q, lastWords = normalize(q).split(' ').filter(Boolean));
const overlaps = (actual, wanted) => !wanted?.length || wanted.some(k => actual.includes(k));
function matchingVariants(product, state, omit = '') {
  if (omit !== 'category' && !overlaps(product.category || [],state.category)) return [];
...
// categories : [{value,label}] dans l'ordre du menu, pour les familles et le catalogue.
function filterCatalog(products, input, pageSize = 60, { categories = [] } = {}) {
  const state = parseFilters(input), facets = {};
  const categoryOrder = new Map(categories.map((c,i)=>[c.value,i]));
  const labels = { category: Object.fromEntries(categories.map(c=>[c.value,c.label])), brand: Object.fromEntries(products.map(p=>[p.brand,p.card.brand])), color: COLORS, material: MATERIALS, usage: USAGES, feature: FEATURES };
  for(const key of LIST_KEYS) {
    const counts = new Map();
    for(const p of products) {
      const variants = matchingVariants(p,state,key); if(!variants.length) continue;
      const values = key === 'category' ? (p.category || []).filter(v=>categoryOrder.has(v)) : key === 'brand' ? [p.brand] : key === 'usage' ? p.usage : key === 'feature' ? p.features : unique(variants.flatMap(v=>v[key]));
    ...
    facets[key] = [...counts].map(([value,count])=>({value,label:labels[key][value] || value,count})).sort(key === 'category'
      ? (a,b)=>(categoryOrder.get(a.value) ?? 1e9)-(categoryOrder.get(b.value) ?? 1e9)
      : (a,b)=>Number(a.value===UNKNOWN)-Number(b.value===UNKNOWN)||a.label.localeCompare(b.label,'fr'));
  }
  ...
  // Les cartes complètes ne sont construites que pour la page affichée (catalogue : 4 000 fiches).
  const all = products.flatMap(p=>{const vs=matchingVariants(p,state);return vs.length ? [{p,vs}] : [];});
  const shownPrice = m => m.price ??= representativeVariants(m.vs,state)[0].price;
  if(state.sort === 'asc') all.sort((a,b)=>shownPrice(a)-shownPrice(b));
  if(state.sort === 'desc') all.sort((a,b)=>shownPrice(b)-shownPrice(a));
  if(state.sort === 'az') all.sort((a,b)=>a.p.card.name.localeCompare(b.p.card.name,'fr'));
  const total = all.length, totalPages = Math.max(1,Math.ceil(total/pageSize));
  state.page = Math.min(state.page,totalPages);
  const items = all.slice((state.page-1)*pageSize,state.page*pageSize).map(m=>resultCard(m.p,m.vs,state));
  return {items,total,totalPages,pageSize,state,facets};
}

module.exports = {buildFilterProduct,variantSearchWords,filterCatalog,parseFilters,filterParams,matchingVariants,resultCard,variantPresentation,colors,materials,measure,normalize,slug};
