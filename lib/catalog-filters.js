...
function buildFilterProduct(raw, card) {
  ...
  const variants = card.variants.map((v, i) => {
    ...
    return { ...v, finishLabel, appearance, partialFinish, color: unique(variantColors).length ? unique(variantColors) : [UNKNOWN], material: mats.length ? mats : [UNKNOWN] };
  });
...
function variantPresentation(v, fallbackImage) {
  ...
  return { variantId: v.id, matchedVariantId: v.id, finishLabel: v.finishLabel, price: v.price, priceIsExact: true,
    image: v.image || fallbackImage, image2: null, compareAt: v.compareAtPrice > v.price ? v.compareAtPrice : null,
...
function resultCard(product, variants, state) {
  ...
  return { id,handle,name,brand,badge,longDelay,leadTimeLabel, ...variantPresentation(chosen, product.card.image),
    priceMin: min, priceMax: max,
    finishChoices: choices.slice(0,4).map(v => variantPresentation(v, product.card.image)), finishCount: choices.length,
