...
function colors(value, brand = '') {
  ...
  if (!result.length && /\b(naturel|natural|chene|oak|hetre|beech|frene|ash|noyer|walnut)\b/.test(text)) result.push('naturel');
  return result;
}
...
function buildFilterProduct(raw, card) {
  ...
  const productMaterial = unique([
    ...
    ...tags.flatMap(t => Object.keys(MATERIAL_WORDS).includes(t) || ['aluminium', 'acier', 'hetre', 'chene', 'rotin', 'polypropylene'].includes(t) ? materials(t) : []),
  ]);
  ...
  const baseMaterials = productMaterial.filter(k => !variableMaterials.includes(k));
  const variants = card.variants.map((v, i) => {
    ...
    return { ...v, color: unique(variantColors).length ? unique(variantColors) : [UNKNOWN], material: mats.length ? mats : [UNKNOWN] };
  });
  return { card, brand: slug(card.brand), usage: usage.length ? usage : [UNKNOWN], features, measurements, variants };
}
...
function resultCard(product, variants, state) {
  const sorted = [...variants].sort((a,b) => Number(b.available) - Number(a.available) || a.price - b.price);
  const chosen = sorted[0];
  const prices = variants.map(v=>v.price);
  const min = Math.min(...prices), max = Math.max(...prices);
  // Prix, image, lien et ajout au panier proviennent de la même variante.
  const priceVariant = variants.filter(v=>v.price === min).find(v=>v.available) || variants.find(v=>v.price === min) || chosen;
  const stockKnown = typeof priceVariant.qty === 'number';
  const anyVariantFilter = state.color.length || state.material.length || state.stock || state.min !== null || state.max !== null;
  ...
  return { id,handle,name,brand,badge,longDelay,leadTimeLabel, variantId: priceVariant.id, matchedVariantId: priceVariant.id,
    price: priceVariant.price, priceMin: min, priceMax: max, compareAt: priceVariant.compareAtPrice > min ? priceVariant.compareAtPrice : null,
    image: priceVariant.image || product.card.image, image2: anyVariantFilter ? null : product.card.image2,
    available: priceVariant.available, purchaseDisabled: !priceVariant.available, inStock: priceVariant.available && stockKnown && priceVariant.qty > 0,
    availabilityLabel: !priceVariant.available ? 'Indisponible' : stockKnown && priceVariant.qty > 0 ? 'En stock' : 'Sur commande',
...
function filterCatalog(products, input, pageSize = 60) {
  ...
  if(state.sort === 'asc') all.sort((a,b)=>a.priceMin-b.priceMin);
  if(state.sort === 'desc') all.sort((a,b)=>b.priceMin-a.priceMin);
...
module.exports = {buildFilterProduct,filterCatalog,parseFilters,filterParams,matchingVariants,colors,materials,measure,normalize,slug};
