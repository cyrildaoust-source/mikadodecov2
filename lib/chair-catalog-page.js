...
function renderChairCatalog(html, data, view, renderCard) {
  ...
  html=html.replace('<html lang="fr">',`<html lang="fr" class="plp-collection" data-chair-catalog${continuation?' data-chair-continuation':''}>`)
    ...
    .replace('<p data-plp-sub>Mobilier de design, choisi pièce par pièce.</p>','<p data-plp-sub>Des matières, des lignes et des couleurs pour trouver la vôtre.</p>')
    ...
    .replace('<div class="wrap" data-breadcrumb>',`<div class="chair-catalog__compact wrap"><p class="lab">Mobilier · Assises</p><h1 class="serif">${esc(title)}</h1></div><div class="wrap" data-breadcrumb>`)
    ...
    .replace('<div class="pgrid" data-grid></div>',()=>`<div class="pgrid" data-grid data-ssr="1">${data.items.length?data.items.map(p=>renderCard(p,view.chairURL(data.state))).join(''):view.emptyChairs()}</div>`)
    .replace('<nav class="plp-pagination" data-pagination aria-label="Pagination" hidden></nav>',()=>`<nav class="plp-pagination" data-pagination aria-label="Pagination des chaises"${data.totalPages<=1?' hidden':''}>${view.chairPagination(data)}</nav>`)
