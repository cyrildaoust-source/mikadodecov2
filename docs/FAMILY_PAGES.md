...
Décision du propriétaire du 24 septembre 2026 : les familles (dont Jardin et Assises) et le catalogue complet reçoivent les filtres « Filtrer et trier » du pilote Chaises, avec un filtre « Catégorie » (sous-catégories du menu sur une famille, familles sur le catalogue) et des pages numérotées de 60 produits. La composition éditoriale ne change pas ; dès la page 2, seuls le titre compact, le fil d'Ariane et la liste restent. Une famille réunit sa collection Shopify et ses sous-catégories du menu, car les collections familles sont incomplètes (Assises ne contenait aucun des 712 canapés). Assises garde sa règle « sans mobilier de jardin » et Tables sa règle intérieur. Voir `docs/CATALOG_FILTERS.md` pour l'index commun.

Liste d'origine, servie seulement si l'index commun n'est pas encore disponible (quelques dizaines de secondes après un déploiement), sans être gardée au CDN : les collections sont chargées par lots de 24, sans plafond global ajouté par ces pages. Le premier lot est rendu côté serveur, avec des liens produits et une pagination qui fonctionne sans JavaScript. « Voir plus de produits » ajoute ensuite les lots dans la même grille. Une panne conserve la navigation et offre une nouvelle tentative ; elle n’est pas mise en cache comme une page de produits normale.
...
Les catalogues à pagination numérotée affichent désormais 60 produits par page, via la constante commune `DISPLAY_PAGE_SIZE` de `v3/catalog-pagination.mjs`. La taille des lots API et le premier rendu serveur ne changent pas : cette valeur règle uniquement le découpage de la liste finale, après filtrage et tri. La même taille est utilisée sur tous les appareils pour garder des URLs cohérentes. Les familles filtrées suivent la même taille de 60 ; seule la liste d'origine de secours garde ses lots de 24.
